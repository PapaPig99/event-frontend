      ...
      <nav class="nav">
        ...
        <div v-if="open.events" class="submenu">
          <RouterLink
            to="/admin/events/all"
            class="sub-item"
            :class="{ active: isActive('/admin/events/all') }"
          >
            <span class="dot" :class="{ on: isActive('/admin/events/all') }"></span>
        ...
        <!-- Events (submenu-Create Event) -->
          <RouterLink
            ...
            class="sub-item"
            :class="{ active: isActive('/admin/events/create') }"
          >
            <span class="dot" :class="{ on: isActive('/admin/events/create') }"></span>
...
.sep {
  width: 2px; height: 22px; background: #fff; display:inline-block;
}
.signout {
...
/* ===== Nav styles ===== */
.nav { padding: 6px 0 18px; font-size: 15px; }
.nav-item {
  width: 100%;
  ...
  border-left: 4px solid transparent;        /* แถบซ้ายเล็ก */
}
.nav-item:hover { background: var(--red-50); }
.nav-item.active {
  color: var(--red);
  background: var(--red-50);
  border-left-color: var(--red);              /* แถบซ้ายสีแดงเมื่อ active */
  font-weight: 600;
}
.nav-item.is-button {
...
.sub-item:hover { background: var(--red-50); }
.sub-item.active {
  color: var(--red); font-weight: 600; border-left-color: var(--red);
}
...
.sidebar::-webkit-scrollbar-thumb { background:#e9e9e9; border-radius: 8px; }
</style>
